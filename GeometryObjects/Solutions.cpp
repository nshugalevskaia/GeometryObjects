...
size_t count_of_points_near_circle(size_t N)
{
	...
	for (int i = 0; i < N; ++i)
	{
		set_points.push_back(Point((double)(rand() + 1) / (RAND_MAX + 2) * (max_x - min_x) + min_x, (double)(rand() + 1) / (RAND_MAX + 2) * (max_y - min_y) + min_y));
	}
...
int main()
{
	std::cout << count_of_points_inside_polygon("input.txt");
	std::cout << count_of_points_near_circle(100);
}